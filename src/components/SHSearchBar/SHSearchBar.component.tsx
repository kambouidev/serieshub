...
const SHSearchBar: FC<SHSearchBarProps> = ({ searchQuery, setSearchQuery, onSearch, lastQueries }) => {
    ...
    const handleInputChange = (e: { target: { value: string; }; }) => {
        ...
        const filteredRecentSearches = lastQueries.filter(recent => recent.toLocaleLowerCase().includes(e.target.value.toLocaleLowerCase()));
    ...
    const handleFocus = () => {
        setFilteredRecentSearches(lastQueries);
        if (searchQuery.length === 0 && lastQueries.length > 0) setShowRecentSearches(true);
    };
